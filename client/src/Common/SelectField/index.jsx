export default class SelectField extends BaseComponent {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.value && (
      ...
      let filterItems = items.filter(
        (item) => item.id === nextProps.value
          && item[nextProps.filterKey] === nextProps.filterValue
      );
  ...
  render() {
    ...
    let menuItems = filteredItems.map((item) => {
      return <mui.MenuItem key={item.id} value={item.id}
        primaryText={item.name}/>
    });
    if (props.withBlankItem) {
      menuItems.unshift(<mui.MenuItem key={0} value={null}
